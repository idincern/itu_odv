#ifndef _ROS_robotnik_msgs_alarmsmonitor_h
#define _ROS_robotnik_msgs_alarmsmonitor_h
...
namespace robotnik_msgs
{
  ...
  class alarmsmonitor : public ros::Msg
  {
    public:
      uint32_t alarms_length;
      typedef robotnik_msgs::alarmmonitor _alarms_type;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(alarms_lengthT > alarms_length)
        this->alarms = (robotnik_msgs::alarmmonitor*)realloc(this->alarms, alarms_lengthT * sizeof(robotnik_msgs::alarmmonitor));
      alarms_length = alarms_lengthT;
      for( uint32_t i = 0; i < alarms_length; i++){
      offset += this->st_alarms.deserialize(inbuffer + offset);
        memcpy( &(this->alarms[i]), &(this->st_alarms), sizeof(robotnik_msgs::alarmmonitor));
      }
     return offset;
    }

    const char * getType(){ return "robotnik_msgs/alarmsmonitor"; };
    const char * getMD5(){ return "e686c3a2b0932118650b57e91f9eee9a"; };
